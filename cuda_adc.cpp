...
std::vector<float> Detector::preprocessImage
(const cv::Mat& image)
{
    ...
    cv::cuda::GpuMat gpu_img, resized_image, floatImage;
    gpu_img.upload(image);
    cv::cuda::cvtColor(gpu_img, gpu_img, cv::COLOR_BGR2RGB);
    cv::cuda::resize(gpu_img, resized_image, cv::Size(input_shape[2], input_shape[3]));
    //resized_image = cuda_resize_img(gpu_img, input_shape[3], input_shape[2]);
    resized_image.convertTo(floatImage, CV_32FC3, 1 / 255.0);


    std::vector<cv::cuda::GpuMat> gpuChannels(3);
    cv::cuda::split(floatImage, gpuChannels);//split source
    std::vector<cv::Mat> channels(3);

    std::vector<float> input_tensor_values;
    for (int c = 0; c < 3; ++c)
    {
        gpuChannels[c].download(channels[c]);
        input_tensor_values.insert(input_tensor_values.end(), (float*)channels[c].data, (float*)channels[c].data + input_shape[2] * input_shape[3]);
    }
